...
write = xlsxwriter.Workbook('D:/大数5/规整后/单体外形.xlsx')
sheet1 = write.add_worksheet('单体外形')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/单体外形.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='CNZZWX')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['CNZZWX']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/单体外形.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/单体外形.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    temp = re.sub('锂.*?\\：','',temp)
    temp = re.sub('电.*?\\：','',temp)
    temp = temp.replace('≥','').replace('，',',').replace(';',',').replace('；',',').replace(' ','').replace('/',',').replace('+',',').replace(',,',',').replace('超级','')
    temp = resymbol(temp)
