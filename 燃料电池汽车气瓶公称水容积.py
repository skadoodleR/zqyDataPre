...
write = xlsxwriter.Workbook('D:/大数5/规整后/燃料电池汽车气瓶公称水容积.xlsx')
sheet1 = write.add_worksheet('燃料电池汽车气瓶公称水容积')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/燃料电池汽车气瓶公称水容积.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='RLDCQPRJ')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['RLDCQPRJ']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/燃料电池汽车气瓶公称水容积.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/燃料电池汽车气瓶公称水容积.xlsx')
