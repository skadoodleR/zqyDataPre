...
write = xlsxwriter.Workbook('D:/大数5/规整后/电池类型.xlsx')
sheet1 = write.add_worksheet('电池类型')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/电池类型.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='DLXDCZL')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['DLXDCZL']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/电池类型.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/电池类型.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    temp = re.sub('\\（.*?\\）','',temp)
    temp = re.sub('状.*?\\:','',temp)
    temp = temp.replace('≥','').replace('，',',').replace('电池:','').replace('电容:','').replace(';',',').replace('；',',').replace(' ','').replace('+',',').replace('/',',')
    temp = resymbol(temp)
