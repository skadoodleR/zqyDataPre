...
write = xlsxwriter.Workbook('D:/大数/规整后/总质量.xlsx')
sheet1 = write.add_worksheet('总质量')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数/原始值/总质量.xlsx', usecols=[0])
M = M.sort_values(by='总质量')
...
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数/index/总质量.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数/index/总质量.xlsx')

for i in range(0, len(indecs)): 
    sheet1.write(i, 0, indecs[i])
    temp = indecs[i]
    temp = temp.replace('(公路)','').replace('(49座)','').replace('(60座)','').replace('(5人)','').replace('(7人)','').replace('(并装双轴','').replace(' ',',').replace('、',',').replace('，',',').replace(';',',').replace('；',',').replace('-',',').replace('),',',').replace(',,',',').replace(')(',',').replace('(',',').replace(')',',')
    temp = resymbol(temp)
