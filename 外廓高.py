...
write = xlsxwriter.Workbook('D:/大数/规整后/外廓高.xlsx')
sheet1 = write.add_worksheet('外廓高')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数/原始值/外廓高.xlsx', usecols=[0])
M = M.sort_values(by='高')
...
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数/index/外廓高.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数/index/外廓高.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    if temp != None and ',' in temp:
        temp = temp.replace(' ',',').replace('、',',').replace('，',',').replace(';',',').replace('；',',').replace('-',',').replace('),',',').replace(',,',',').replace(')(',',').replace('(',',').replace(')',',').replace(',装顶行李架','').replace('.',',').replace(',带空调','').replace('OO','00').replace(',选装时','')
