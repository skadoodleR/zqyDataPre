...
write = xlsxwriter.Workbook('D:/大数5/规整后/单体1小时率额定容量.xlsx')
sheet1 = write.add_worksheet('单体1小时率额定容量')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/单体1小时率额定容量.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='DLXDCLEC1')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['DLXDCLEC1']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/单体1小时率额定容量.xlsx')
indecs = indecs['indecs']
...
# -------------------------查之前存在的xlsx文件
indecs.to_excel('D:/大数5/index/单体1小时率额定容量.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    temp = re.sub('\\（.*?\\）','',temp)
    temp = re.sub('\\(.*?\\）','',temp)
    temp = temp.replace('≥','').replace('，',',').replace(';',',').replace('；',',').replace(' ','').replace('Ah','').replace('/',',').replace('、',',')
    temp = resymbol(temp)
