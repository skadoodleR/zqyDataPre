...
write = xlsxwriter.Workbook('D:/大数5/规整后/单体质量.xlsx')
sheet1 = write.add_worksheet('单体质量')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/单体质量.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='CNZZDTZL')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['CNZZDTZL']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/单体质量.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/单体质量.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    temp = re.sub('电.*?\\:','',temp)
    temp = re.sub('F.*?\\:',',',temp)
    temp = temp.replace('≥','').replace('，',',').replace(';',',').replace('；',',').replace(' ','').replace('/',',').replace('≤',',').replace('+/-','±').replace('+',',').replace('和',',').replace(',,',',').replace('超级','').replace('锂电池：','').replace('或',',').replace('g','').replace('华霆:','').replace('国轩:','').replace('状态2:','')
    temp = resymbol(temp)
