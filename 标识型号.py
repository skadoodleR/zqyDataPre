...
write = xlsxwriter.Workbook('D:/大数/规整后/标识型号.xlsx')
sheet1 = write.add_worksheet('标识型号')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数/原始值/标识型号.xlsx', usecols=[0])
M = M.sort_values(by='标识型号')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['标识型号']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数/index/标识型号.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数/index/标识型号.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    if temp != None:
        temp = temp.replace(';',',').replace('；',',').replace(' ',',').replace(',,,',',').replace(',,',',').replace('或',',').replace('/',',')
