...
write = xlsxwriter.Workbook('D:/大数5/规整后/驱动电机峰值功率转速转矩.xlsx')
sheet1 = write.add_worksheet('驱动电机峰值功率转速转矩')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/驱动电机峰值功率转速转矩.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='DDQCGLZSZJ')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['DDQCGLZSZJ']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/驱动电机峰值功率转速转矩.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/驱动电机峰值功率转速转矩.xlsx')
