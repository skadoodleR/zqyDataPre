...
write = xlsxwriter.Workbook('D:/大数/规整后/挂车质量.xlsx')
sheet1 = write.add_worksheet('挂车质量')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数/原始值/挂车质量.xlsx', usecols=[0])
M = M.sort_values(by='挂车质量')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['挂车质量']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数/index/挂车质量.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数/index/挂车质量.xlsx')

for i in range(0, len(indecs)): 
    sheet1.write(i, 0, indecs[i])
    temp = indecs[i]
    temp = re.sub('\\(.*?\\)','',temp)
    temp = temp.replace(' ',',').replace('-',',').replace('`','').replace('/',',').replace('、',',').replace('，',',').replace(';',',').replace('；',',').replace(',,,',',').replace('),',',').replace(',,',',').replace(')(',',').replace('(',',').replace(')',',')
    temp = resymbol(temp)
