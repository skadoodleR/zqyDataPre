...
write = xlsxwriter.Workbook('D:/大数5/规整后/等速法里程.xlsx')
sheet1 = write.add_worksheet('等速法里程')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/等速法里程.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='DDQCXSLC')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['DDQCXSLC']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/等速法里程.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/等速法里程.xlsx')
...
for i in range(0, len(indecs)): 
    ...
    temp = re.sub('配.*?\:','',temp)
    temp = re.sub('配.*?\：','',temp)
    temp = re.sub('D.*?程','',temp)
    temp = temp.replace('≥','').replace('，',',').replace(';',',').replace('；',',').replace(' ','').replace('~',',').replace('km/h','').replace('km','').replace('60km/h','').replace('氢系统工作时的续驶里程','').replace('电量','').replace('Km/h','')
    temp = resymbol(temp)
