...
write = xlsxwriter.Workbook('D:/大数/规整后/前排乘客.xlsx')
sheet1 = write.add_worksheet('前排乘客')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数/原始值/前排乘客.xlsx', usecols=[0])
M = M.sort_values(by='前排乘客')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['前排乘客']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数/index/前排乘客.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数/index/前排乘客.xlsx')
