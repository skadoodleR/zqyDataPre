...
write = xlsxwriter.Workbook('D:/大数5/规整后/电动汽车30分钟最高车速.xlsx')
sheet1 = write.add_worksheet('电动汽车30分钟最高车速')
print('郭睿')
# ---------------------------methods---------------
def resymbol(str):
...
M = pd.read_excel('D:/大数5/电动汽车30分钟最高车速.xlsx', usecols=[0],header = 0)
M = M.sort_values(by='DDQCZGCS')
M = M.dropna(axis=0, how='all')
M = M.drop_duplicates()
M = M.reset_index()['DDQCZGCS']
# indecs = pd.DataFrame(columns=['index', 'indecs'])
# indecs = indecs['indecs']
# indecs.loc[0] = '-'
indecs = pd.read_excel('D:/大数5/index/电动汽车30分钟最高车速.xlsx')
indecs = indecs['indecs']
...
# # -------------------------查之前存在的xlsx文件
# indecs.to_excel('D:/大数5/index/电动汽车30分钟最高车速.xlsx')
